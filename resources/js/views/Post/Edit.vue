<template>
    <div class="post-edit">
        ...
        <el-form ref="post" :model="post" label-width="120px">
            ...
            <el-form-item>
                <el-button type="primary" @click="postUpdate">Update</el-button>
        ...
        methods: {
            ...
            getPost(id) {
                api.call('get', 'posts/' + id)
                    .then(({data}) => {
                        ...
                        for (let tag of data.data.post.tags) {
                            tags.push(tag.id);
                        }
                        for (let cat of data.data.post.cats) {
                            cats.push(cat.id);
                        }

                        data.data.post.tags = tags;
                        data.data.post.cats = cats;
            ...
            postUpdate() {
                this.$refs['post'].validate((valid) => {
                    ...
                    api.call('post', 'posts/' + this.$route.params.id, data)
                        .then(({data}) => {
